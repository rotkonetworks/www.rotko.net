...
penumbra's response: figure out engineering it takes to abstract the problem away.

## Decaf377: making attacks impossible

instead of trusting developers to rawdog BLS12-377's complexity, penumbra built Decaf377; a
prime-order group abstraction based on [Ristretto](https://ristretto.group/):
...
team chose for "impossible to fuck up" over "theoretically correct if implemented perfectly".
...
entire field arithmetic later for example upgrade to post-quantum curves.
...
this is as good as trusted setups get: trust one party out of thousands, or alternatively verify the
deployed math yourself. the whole development process was fundamentally public going through almost
testnets prior the launch. even the testnets were mostly run on transparent values, encryption
added only last once there was certainty that the protocol was functional in secure manner.
...
audit findings: implementation bugs, not design flaws. the defensive layers worked - bugs caused
panics (fail-closed) instead of silent vulnerabilities (fail-open). turns out building abstraction
layers that make entire bug classes impossible actually prevents those bugs.
