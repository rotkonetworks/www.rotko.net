...
monitor the mempool, captures MEV. MEV extraction creates economic power structures that operate
parallel to formal governance, with validators and block builders wielding significant influence
through their ability to reorder and extract value from transactions.
...
instead of allowing validators to extract MEV, penumbra's protocol performs arbitrages itself.
all orders are batched together, and the protocol calculates a fair average price rather than
executing them sequentially. any value that would have been MEV gets burned, removing the
extraction mechanism entirely. there's no transaction ordering advantage because there's no
meaningful ordering—just batched execution at protocol-determined prices.
