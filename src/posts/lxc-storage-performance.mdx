...
One of our Kusama validators (val-kusama-03) was experiencing 2x slower storage performance. The
issue: Proxmox LXC directory-backed storage with ext4-over-ext4 configuration. Moving to LVM
resolved it immediately.

## the problem

Proxmox LXC defaults to directory-backed storage. This creates an architecture that's convenient for
setup but disastrous for fsync-heavy workloads:
...
**Proxmox assumes ZFS, not ext4**: Directory-backed storage with ZFS host filesystem doesn't have
the same double-ext4-journaling problem. If you're running ext4 on the host, use LVM instead.
...
**Migration effort**: 3 hours including validation
